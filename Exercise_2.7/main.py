...
if train_single_SNR:
    ...
    if ce_type == 'dnn':
        sess, input_holder, output = networks.build_ce_dnn(K, SNR_train, training_epochs=training_epochs, batch_size=batch_size,
                                                               savefile='dnn_ce/CE_DNN_'+ ('CPFREE_' if CP_flag is False else '') +
                                                                        str(2 ** mu) + 'QAM_SNR_' + str(SNR_train[0]) + str(SNR_train[-1]) + 'dB.npz', test_flag=test_ce, cp_flag=CP_flag, nh1=500, nh2=250)
