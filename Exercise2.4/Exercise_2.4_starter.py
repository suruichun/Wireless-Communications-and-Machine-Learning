...
def generate_real_samples_with_labels_Rayleigh(h_dataset, number=100):
    ...
    # 1. randomly select channel
    h = np.random.choice(h_dataset, number)

    # 2. randomly generate QAM symbols
    labels_index = np.random.randint(0, len(mean_set_QAM), number)
    data_t = mean_set_QAM[labels_index]

    # 3. simulate channel y = hx + n
    transmit_data = h * data_t

    transmit_data = np.hstack((
        np.real(transmit_data).reshape(number,1),
        np.imag(transmit_data).reshape(number,1)
    ))
