...
def train(config):    
    ...
    Pilot_file_name = 'Pilot_'+str(P) if mu==2 else 'Pilot_'+str(P)+'_'+str(mu)
    ...
    print(bits.shape, pilotValue.shape)
    CP_flag = config.with_CP_flag
    ...
    # Network Parameters
    n_hidden_1 = config.hiddendims[0]
    n_hidden_2 = config.hiddendims[1] # 1st layer num features
    n_hidden_3 = config.hiddendims[2] # 2nd layer num features
    n_input = 256 # MNIST data input (img shape: 28*28)
    n_output = 1 + config.pred_range[-1] - config.pred_range[0]#16 #4
    ...
    with tf.Session(config=config_GPU) as sess:
        ...
        for epoch in range(training_epochs):
            ...
            if epoch % model_saving_step == 0:
                saving_folder = config.Model_path + 'SNR_' + str(SNRdb) + '_PredRange_' + str(config.pred_range[0]*1000+config.pred_range[-1])
                saving_name = saving_folder + '/DetectionModel_SNR_' + str(SNRdb) + '_Pilot_' + str(P) + '_epoch_' + str(epoch)
...
def test(config):   
    ...
    if not os.path.exists(csv_path):
        df = pd.DataFrame(columns=["Num_P", "SNR", "mu", "r1", "r2", "BER"])
    ...
    with tf.Session(config=config_GPU) as sess:
        sess.run(init)
        saving_folder = config.Model_path + 'SNR_' + str(SNRdb) + '_PredRange_' + str(config.pred_range[0]*1000+config.pred_range[-1])
        saving_name = saving_folder + '/DetectionModel_SNR_' + str(SNRdb) + '_Pilot_' + str(P) + '_epoch_' + str(110)
        ...
        BER = 1-tf.reduce_mean(tf.reduce_mean(tf.to_float(tf.equal(tf.sign(y_pred-0.5), tf.cast(tf.sign(batch_y-0.5),tf.float32))),1))               
        # print("OFDM Detection QAM output number is", n_output, "SNR = ", SNRdb, "Num Pilot", P,"prediction and the mean error on test set are:", mean_error.eval({X:batch_x}), mean_error_rate.eval({X:batch_x}))
        ber = BER.eval({X:batch_x})
        print("BER on test set ", ber) 
        # 根據 label append
        new_row = pd.DataFrame([{
            "Num_P": P,
            "SNR": SNRdb,
            "mu": mu,
            "r1": config.pred_range[0], 
            "r2": config.pred_range[-1], 
            "BER": ber
        }])
