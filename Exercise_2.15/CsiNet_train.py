...
K.set_image_data_format('channels_last')   # Force NHWC globally
...
def residual_network(x, residual_num, encoded_dim):
    ...
    x = Reshape((img_height, img_width, img_channels, ))(x)  # Reshape back to 4D CSI tensor
    for i in range(residual_num):
        x = residual_block_decoded(x)  # Stack residual blocks for reconstruction
    
    x = Conv2D(2, (3, 3), activation='sigmoid', padding='same', data_format="channels_last")(x)  # Output [0,1] for real/imag parts
    return x

# Build input tensor and full autoencoder model
image_tensor = Input(shape=(img_height, img_width, img_channels))  # Input shape for CSI data
...
x_train = np.reshape(x_train, (len(x_train), img_channels, img_height, img_width)).transpose((0, 2, 3, 1))
x_val = np.reshape(x_val, (len(x_val), img_channels, img_height, img_width)).transpose((0, 2, 3, 1))
x_test = np.reshape(x_test, (len(x_test), img_channels, img_height, img_width)).transpose((0, 2, 3, 1))
